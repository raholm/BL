...
for (i in 1:ncol(betas)) {
    ci <- quantile(betas[, i], c(0.015, 0.985))
    xlim <- c(min(min(betas[,i]), quadbetas[i]), max(max(betas[,i]), quadbetas[i]))

    hist(betas[, i], xlim=xlim, xlab="", ylab="", main=paste("Beta", i),
         breaks=50, xaxt='n', yaxt="n")
    abline(v=ci[1], lwd=4, col="blue")
    abline(v=ci[2], lwd=4, col="blue")
    abline(v=quadbetas[i], lwd=4, col="red")
}
```
