...
metropolis_hastings <- function(prop_sampler, log_prop_func, log_targ_post_func,
                                X0, iters, ...){
    x <- X0
...
The parameters seem to converge but some of them might have some small autocorrelation left in them. Plots with cummulative means will be assesed.
...
The cummulative means seem to be quite stable although compleatly converged. More itterations are needed to get a more stable result.
...
for (i in 1:9){
  ...
  hist(metro_res[,i], freq = FALSE, breaks = 50, main = expression(beta[paste(i)]),
       xlab = " ")
...
for (i in 1:9){
  ...
  hist(exp(metro_res[,i]), freq = FALSE, breaks = 50, main = expression(exp(beta[paste(i)])),
       xlab = " ")
