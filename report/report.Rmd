...
plot(x = 1:nDraws, y = betamean, type = "l",
     xlab="# of Samples", ylab="Expected Value")
abline(h = true_mean, col = "red", lwd = 2)
```

The posterior mean for the beta distribution seem to be converging at the beta distributions expected mean for the beta distribution. (represented by the red line in the graph above).
...
The posterior standard deviation also converges at the expected standard deviation as expected.

```{r}
hist(samples, freq=FALSE, breaks=50,
     xlab="theta", main="Histogram of Samples", xlim=c(0, 1))
...
```
The posterior probability for theta beeing below 0.4 is calculated as the share of values under 0.4.

$Pr(\theta < 0.4 |y) \approx$ `r est_04`
And the true value being 
...
hist(sample_odds, breaks=50, freq = FALSE)
...
For the Highest Posterior Density (HPD) Intervall we are with 95 probability certain that the most probable G-value. 
...
plot(sample_exp, post_mises, type="l", ylab="Probability", xlab="Value")
```

So what we do here is that we create a grid of K-values from 0.01 to 10 and use these as the randomly drawn values from the exponential so that we get a probability for each y given all of these K values. To then compute the likelihood we can simply take the product of all of the 10 probabilities for the lambda. 

These sequenced Ks are then also used in the prior but as points of density in the exponential distribution.  
