...
betas <- probitgibbs(X = women[,-1], Y = women[,1],
                     ...
                     beta = rep(0, times = ncol(women[,-1])), iter = 1000)
...
logposterior <- function(beta, X, Y, mu, sigma){
   loglikelihood(beta, X, Y) + logprior(beta, mu, sigma)
}
...
params <- matrix(c(coef(glm_res), normal_res$par, colMeans(metro_res)), nrow=3, byrow=TRUE)
colnames(params) <- c("Beta0", "Beta1", "Beta2", "Beta3", "Beta4", "Beta5", "Beta6", "Beta7", "Beta8")
rownames(params) <- c("glm", "normal", "metropolis")
kable(params, format="latex", digits=3)
```
...
for (i in 1:9){
    plot(x = 1:nrow(metro_res), y = metro_res[,i], type = "l",
...
The parameters seem to converge but some of them might have some small autocorrelation left in them. Plots with cumulative means will be assessed.

```{r, fig.width=8, fig.height=8}
metro_cummean <- apply(metro_res, MARGIN = 2, FUN = function(x) cumsum(x) / (1:length(x)))

old <- par(mfrow =c(3,3))

for (i in 1:9){
    plot(x = 1:nrow(metro_cummean), y = metro_cummean[,i], type = "l",
         xlab="iteration", ylab=bquote(beta[.(i-1)]))
}
...
The cumulative means seem to be quite stable although not completely converged. More iterations are needed to get a more stable result.

```{r, fig.width=8, fig.height=10}
old <- par(mfrow =c(3,3))

for (i in 1:9){
    hist(metro_res[,i], freq = FALSE, breaks = 50,
         main =bquote(beta[.(i-1)]), xlab="", axes=TRUE)
  lines(density(metro_res[,i]), col = "blue")
}
...
for (i in 1:9){
    hist(exp(metro_res[,i]), freq = FALSE, breaks = 50,
         main = bquote(exp(beta[.(i-1)])), xlab="")
  lines(density(exp(metro_res[,i])), col = "blue")
}
...
hist(predsamples, breaks = 50, main="Predictive Distribution", xlab="Number of bids")
```

```{r}
paste("The probability that there are no bidders", mean(predsamples == 0))
```
