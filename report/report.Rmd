...
sigma2sampler <- function(x, I, hyperparams){
    n <- nn(I)
    d <- dd(x, I)
    ...
    sigma1 <- rinvchisq(1, vn1, sn1)
    sigma2 <- rinvchisq(1, vn2, sn2)
...
mu2sampler <- function(x, I, sigmasq, hyperparams){
    ...
    tau1 <- taun(d$d1, sigmasq[1], hyperparams)
    mun1 <- mun(d$d1, sigmasq[1], hyperparams)

    tau2 <- taun(d$d2, sigmasq[2], hyperparams)
    mun2 <- mun(d$d2, sigmasq[2], hyperparams)

    mu1 <- rnorm(1, mun1, tau1)
    mu2 <- rnorm(1, mun2, tau2)
...
mixedsamples <- mixedgibbs(rainfall$V1, iter, init, hyperparams)
```

```{r}
par(mfrow = c(2,2))
plot(x = 1:iter, mixedsamples[,1], type = "l")
plot(x = 1:iter, mixedsamples[,2], type = "l")
plot(x = 1:iter, mixedsamples[,3], type = "l")
plot(x = 1:iter, mixedsamples[,4], type = "l")
par(mfrow = c(1,1))
```
